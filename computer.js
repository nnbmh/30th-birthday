...
/* =========================================================
   PRE-WARM COMPUTER CONTENT
   Makes first clicks behave like already-opened content
   ========================================================= */

function prewarmComputerContent() {
  /* Preload every Pictures thumbnail immediately */
  ...
  thumbnailPaths.forEach((src) => {
    const img = new Image();
    img.src = src;
  });

  /*
    Build the heavier computer screens invisibly once.

    The browser now has to do the HTML/layout work BEFORE
    Faris actually clicks the files.
  */
  ...
  warmup.innerHTML = `
    <div class="birthday-pc-file">
      ${computerAppData.birthday.content}
    </div>

    <div class="birthday-pc-file">
      ${computerAppData.message.content}
    </div>

    <div class="birthday-pc-file">
      ${computerAppData.classified.content}
    </div>

    <div class="prewarm-pictures">
      ${renderPhotosHome()}
  ...
  /*
    Force the browser to calculate the hidden content once.
  */
  void warmup.offsetHeight;

  /*
    Wait for the thumbnail files to finish decoding as well.
  */
  const images = Array.from(warmup.querySelectorAll("img"));
  ...
  Promise.all(
    ...
    /*
      Keep it briefly so the browser finishes layout/paint work,
      then remove the invisible copy.
    */
    setTimeout(() => {
      warmup.remove();
    }, 1000);
...
/*
  Start warming everything as soon as computer.js has loaded.
*/
prewarmComputerContent();
